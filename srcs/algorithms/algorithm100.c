...
/*   Updated: 2023/09/23 19:55:05 by becamino         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void 	ft_push_b(t_list **stack_a, t_list **stack_b, int stack_len, int bigger_order)
{
	int		count;
	int		i;
	int		next;
	...
	i = 0;
	count = ft_counter_bign(stack_a, bigger_order);
	while (i < count)
	{
		if ((*stack_a)->order <= bigger_order)
		{
			pb(stack_a, stack_b);
			i++;
		}
